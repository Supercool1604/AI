...
def main():
	...
	if(startNode not in nodesAndActions):
		f = open('output.txt', 'w+')
		f.write("FAIL")
		return

	if(destinationNode[0] >= environmentLimits[0] or destinationNode[1] >= environmentLimits[1] or destinationNode[2] >= environmentLimits[2]):
		f = open('output.txt', 'w+')
		f.write("FAIL")
		return

	if(startNode[0] >= environmentLimits[0] or startNode[1] >= environmentLimits[1] or startNode[2] >= environmentLimits[2]):
		f = open('output.txt', 'w+')
		f.write("FAIL")
		return
